...
<body>
    ...
    <table id="tabela-kursanteve" class="table table-striped table-bordered table-sm">
        ...
        <tr>
            <?php $sqlquery = "SELECT * FROM kursantet WHERE Statusi='pabere' AND Dega = '$degaid' LIMIT 100";
            ...
            <td class="text-left">
                <?php echo decrypt($row['Amza']); ?>
            </td>
            <td class="text-left">
                <?php echo decrypt($row['NrSerisDeshmis']); ?>
            </td>
            <td class="text-left">
                <?php echo $row['Datakursit']; ?>
            </td>
            <td class="text-left"><button class="btn btn-success"
                    onclick="location.href = '../methods/ndryshoamzen.php?id=<?php echo $row['ID']; ?>'">Ploteso
                    Amzen</button><button class="btn btn-secondary"
                    onclick="location.href = '../methods/munges.php?id=<?php echo $row['ID']; ?>'">Mungoi</button><button
                    class="btn btn-danger"
                    onclick="location.href = '../methods/fshirregjistrimin.php?id=<?php echo $row['ID']; ?>'">Fshi</button>
            </td>
        </tr>
        <?php } ?>
    </table>
